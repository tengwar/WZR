...
void PoczatekInterakcji()
{
   ...
   //       &dwThreadId);                // returns the thread identifier         
	...
	unicast_net *uni_send = new unicast_net(1002);

	while (1)
	{
		Ramka ramka;
		unsigned long adr;
		short whatever = sizeof(Ramka);
		int rozmiar;                                 // liczba bajtów ramki otrzymanej z sieci
		uni_recv->reciv((char*)&ramka, &adr, whatever);
		StanObiektu stan;
		stan = ramka.stan;
			...
			// stworzyæ
		{
			...
			adresy[iLiczbaInnychOb] = adr;
			// u³atwia wyszukiwanie, alternatyw¹ mo¿e byæ tabl. rozproszona           
			printf("zarejestrowano %d obcy obiekt \n", iLiczbaInnychOb - 1);
