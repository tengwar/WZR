...
enum typy_ramek {STAN_OBIEKTU, WZIECIE_PRZEDMIOTU, ODNOWIENIE_SIE_PRZEDMIOTU, KOLIZJA, PRZEKAZ, 
                 PROSBA_O_ZAMKNIECIE, NEGOCJACJE_HANDLOWE, ZADANIE_WSPOLPRACY, AKCEPTACJA_WSPOLPRACY, POTW_WSPOLPRACY, ODMOWA_WSPOLPRACY};
...
DWORD WINAPI WatekOdbioru(void *ptr)
{
  ...
  while(1)
  {
    ...
    case ZADANIE_WSPOLPRACY:
    {
							   
							   int numerNadawcy = ramka.iID_nadawcy;
							   if (numerNadawcy != pMojObiekt->iID)
							   {
								   char * nadawcaStr = new char[10];
								   itoa(numerNadawcy, nadawcaStr, 10);
								   int odpowiedz = MessageBoxA(okno, nadawcaStr, "Chcesz wspolpracowac?", 1);
								   if (odpowiedz == 1)
								   {
									   Ramka ramka;
									   ramka.typ_ramki = AKCEPTACJA_WSPOLPRACY;
									   ramka.stan = pMojObiekt->Stan();
									   ramka.iID_adresata = numerNadawcy;
									   ramka.iID_nadawcy = pMojObiekt->iID;
									   multi_send->send((char*)&ramka, sizeof(Ramka));
								   }
								   else
								   {
									   Ramka ramka;
									   ramka.typ_ramki = ODMOWA_WSPOLPRACY;
									   multi_send->send((char*)&ramka, sizeof(Ramka));
								   }
							   }
    ...
    case AKCEPTACJA_WSPOLPRACY:
    {
								  ...
								  int nadawca = ramka.iID_nadawcy;
								  if (nadawca != pMojObiekt->iID && ramka.iID_adresata == pMojObiekt->iID)
								  {
									  char * nadawcaStr = new char[10];
									  itoa(nadawca, nadawcaStr, 10);
									  int odpowiedz = MessageBoxA(okno, "Czy chcesz wspolpracowac z tym pojazdem?", nadawcaStr, 1);
									  if (odpowiedz == 1)
									  {
										  Ramka ramka2;
										  ramka2.typ_ramki = POTW_WSPOLPRACY;
										  ramka2.nr_druzyny = pMojObiekt->iID;
										  ramka2.iID_adresata = nadawca;
										  ramka2.iID_nadawcy = pMojObiekt->iID;
										  pMojObiekt->ID_druzyny = pMojObiekt->iID;
										//  if (IndeksyOb[stan.iID] != -1)
										//	  CudzeObiekty[IndeksyOb[stan.iID]]->ZmienStan(stan);
										  CudzeObiekty[IndeksyOb[ramka.iID_nadawcy]]->ID_druzyny = pMojObiekt->iID;
									  ...
									  else
									  {
										  Ramka ramka;
										  ramka.typ_ramki = ODMOWA_WSPOLPRACY;
										  ramka.iID_adresata = nadawca;
										  multi_send->send((char*)&ramka, sizeof(Ramka));
									  }
								  }
                               break;
    }
    case POTW_WSPOLPRACY:
    {
							if (ramka.iID_adresata == pMojObiekt->iID)
							{
								char * druzyna = new char[10];
								itoa(ramka.nr_druzyny, druzyna, 10);
								pMojObiekt->ID_druzyny = ramka.nr_druzyny;
								CudzeObiekty[IndeksyOb[ramka.iID_nadawcy]]->ID_druzyny = ramka.nr_druzyny;
								MessageBox(okno, "Jestes w druzynie", druzyna, 1);
							}

                            
                                  break;
    }
	case ODMOWA_WSPOLPRACY:
	{
							  if (ramka.iID_adresata == pMojObiekt->iID)
							  {
								  MessageBox(okno, "Jednak nie chce wspolpracowac", ":<", 2);
							  }
							break;
	}
...
void Cykl_WS()
{
  ...
  if (pMojObiekt->wyslij_prosbe)
  {
      Ramka ramka;
      ramka.typ_ramki = ZADANIE_WSPOLPRACY;
      ramka.iID_nadawcy = pMojObiekt->iID;
      int iRozmiar = multi_send->send((char*)&ramka, sizeof(Ramka));
	  pMojObiekt->wyslij_prosbe = false;
  }
