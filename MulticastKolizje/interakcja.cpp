...
DWORD WINAPI WatekOdbioruKom(void *ptr)
{
	...
	while (1)
	{
		...
		if (stan.iID != pMojObiekt->iID)          // jeœli to nie mój w³asny obiekt
		{
			...
			{
				if (pMojObiekt->Kolizja(*InneObiekty[i]))
				{
					pMojObiekt->zawroc = true;
					break;
				}
			}
		}
...
void Cykl_WS()
{
	...
	if (pMojObiekt->zawroc)
	{
		pMojObiekt->wV.x = -pMojObiekt->wV.x;
		pMojObiekt->wV.y = -pMojObiekt->wV.y;
		pMojObiekt->wV.z = -pMojObiekt->wV.z;
		pMojObiekt->zawroc = false;
	}
